...
class LoginActivity : RxAppCompatActivity() {
    ...
    private fun authenticate(uri: Uri?) {
        if (uri != null && uri.toString().startsWith(redirectUri)) {
            ...
            Toast.makeText(this.applicationContext,code,Toast.LENGTH_SHORT).show()
            Log.d("CODE",code);
            hideAll()
    ...
    private fun handleSuccess(): Consumer<KeycloakToken> {
        return Consumer { token ->
            ...
            Toast.makeText(this.applicationContext,"access_token: "+token.accessToken,Toast.LENGTH_SHORT).show()
